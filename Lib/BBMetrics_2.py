...
class BarneyBotTripletClassifier:
    ...
    def compute(self, sentences, character, character_dict, base_folder, n_draws):
        ...
        if n_samples < 3:
            raise Exception("Please deploy at least 3 sentences to run the metric.")
#         elif n_samples <= self.max_samples:
#             triplets = [list(triplet) for triplet in itertools.permutations(samples, 3)]
#             print(type(triplets))
        else:
            triplets = []
            for i in range(n_samples-2):
                triplets.append([samples[i], samples[i+1], samples[i+2]])
