...
class BarneyBotTripletClassifier:
    ...
    def compute(self,
        ...
        character_folder = os.path.join(base_folder, "Data", "Characters",
                                        character)
        checkpoint_folder = os.path.join(
            character_folder, character_dict[character]['classifier_name'])
    ...
    def train(self,
        ...
        model_path = os.path.join(character_folder,
                                  character_dict[character]['classifier_name'])
        ...
        if not os.path.exists(
                os.path.join(
                    character_folder,
                    character_dict[character]['encoded_lines_filename'])):
        ...
        if not from_saved_embeddings:
            ...
            series_df = pd.read_csv(
                os.path.join(source_folder,
                             source_dict[source]['df_filename']))
            ...
            series_df[['line', 'character']].to_csv(os.path.join(
                character_folder, character_dict[character]['classifier_df']),
                                                    index=False)
        ...
        series_df['encoded_line'] = np.load(os.path.join(
            ...
            character_dict[character]['encoded_lines_filename']),
                                            allow_pickle=True)
        print(
            "Loaded encoded lines from",
            os.path.join(character_folder,
                         character_dict[character]['encoded_lines_filename']))
        ...
        with open(os.path.join(character_folder, filename),
                  'w',
...
class BBMetric:
    ...
    metrics_list = [
        "bleu", "semantic similarity", "rouge l", "emotion",
        "semantic answer similarity", "distinct", "semantic classifier",
        "perplexity", "human - coherence", "human - consistency",
        "human - style"
    ]
    ...
    def __init__(self, name, metric):
        ...
        if name == "bleu":
            ...
            self.compute_require_args = set(
                ["sentences", "character", "character_dict", "base_folder"])
            self.compute_optional_args = set(["n_sentences", "verbose"])
            self.train_require_args = set([
                "character", "character_dict", "source_dict", "random_state",
                "base_folder"
            ])
            self.train_optional_args = set(
                ["from_saved_embeddings", "shutdown_at_end", "n_shuffles"])
            self.return_args = ['score', 'std']
            ...
            self.compute_optional_args = set()
            self.train_require_args = set(
                ["model", "tokenizer", "filepath", "questions"])
            self.train_optional_args = set()
            self.return_args = ['score', 'std']
    ...
    def load_metric(name, **kwargs):
        ...
        if name == "bleu":
            ...
            metric = BBMetric(
                name, lambda m, t, f, train, l: human_conversation(
                    m, t, f, train, l))
        elif name == "human - consistency":
            metric = BBMetric(
                name,
    ...
    def compute(self, **kwargs):
        ...
        # Compute perplexity or human metrics by simply passing params
            ...
            outputs = self.metric.compute(
                sentences, kwargs['character'], kwargs['character_dict'],
                kwargs['base_folder'], n_sentences,
    ...
    def train(self, **kwargs):
        ...
        # Otherwise, train the given metric, simply passing the required params
            ...
            self.metric.train(
                kwargs['character'], kwargs['character_dict'],
                kwargs['source_dict'], kwargs['random_state'],
                kwargs['base_folder'],
                kwargs['n_shuffles'] if 'n_shuffles' in kwargs else 10,
                kwargs['shutdown_at_end'] if 'shutdown_at_end' in kwargs else
                False, kwargs['from_saved_embeddings']
                if 'from_saved_embeddings' in kwargs else True)
        elif self.name == "human - coherence":
