...
class BBMetric:
    ...
    metrics_list = [
        ...
        "neural chatbot classifier", "distil-bert classifier", "perplexity",
        "repetitiveness", "term error rate", "bertscore", "comet", "bleurt",
    ...
    def __init__(self, name, metric):
        ...
        elif name == "distil-bert classifier":
            self.compute_require_args = set(["sentences"])
            self.compute_optional_args = set(["verbose"])
            self.train_require_args = set(["characters_path", "model_path"])
            self.train_optional_args = set([
    ...
    def load_metric(name, **kwargs):
        ...
        if name == "google bleu":
            ...
            metric = BBMetric(
                name, ExtendedEditDistance(return_sentence_level_score=True))
        elif name == "t5 grammar correction edit distance":
            metric = BBMetric(
                name,
            ...
            metric = BBMetric(name, DistilBertClassifier(**kwargs))
        else:
    ...
    def compute(self, **kwargs):
        ...
        if self.name == "google bleu":
            ...
            for i in range(len(predictions)):
                single_outputs.append(
                    self.metric(predictions[i], references[i]).item())
            result['score'] = np.mean(np.array(single_outputs))
            result['std'] = np.std(np.array(single_outputs))
        ...
        elif self.name == "distil-bert classifier":
            # Cast sentences as a list
            sentences = kwargs['sentences'] if type(
                kwargs['sentences']) is list else [kwargs['sentences']]
